...
void Port_Init(const Port_ConfigType * ConfigPtr) {
#if(PORT_DEV_ERROR_DETECT == STD_ON) 
  /* CHECK IF THE INPUT CONGIFURATON IS NOT NULL PTR */  
  ...
  { 
    ...
    Port_Configuration = ConfigPtr -> Channels ;  /* address first place in the container */ 
  }
  for(uint8 i = 0; i < CONFIGURED_PORT_PINS ; i++){
    ...
    */
    ...
    if(((Port_Configuration[i].port_num == 3) && (Port_Configuration[i].pin_num == 7)) || ((Port_Configuration[i].port_num == 5 ) && (Port_Configuration[i].pin_num == 0))) {
    ...
    else if ( (Port_Configuration[i].port_num == 2) && (Port_Configuration[i].pin_num <= 3 ))  {
    ...
    if(Port_Configuration[i].direction == PORT_PIN_IN) {
      
      CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET) , Port_Configuration[i].pin_num);   
      if(Port_Configuration[i].resistor_state == PULL_UP) {
        SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PULL_UP_REG_OFFSET) , Port_Configuration[i].pin_num); 
      } 
      else if(Port_Configuration[i].resistor_state == PULL_DOWN) {
        SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PULL_DOWN_REG_OFFSET ) , Port_Configuration[i].pin_num);  
      }
      else  {
        CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PULL_DOWN_REG_OFFSET ) , Port_Configuration[i].pin_num);   
        CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PULL_UP_REG_OFFSET) , Port_Configuration[i].pin_num);  
      }
      
    } 
    else if (Port_Configuration[i].direction == PORT_PIN_OUT) {
      
      SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET) , Port_Configuration[i].pin_num);   
      if(Port_Configuration[i].value == STD_HIGH) {
        SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DATA_REG_OFFSET  ) , Port_Configuration[i].pin_num);  
      } 
      else {
        CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DATA_REG_OFFSET ) , Port_Configuration[i].pin_num);  
      }
...
void Port_SetPinDirection( Port_PinType Pin, Port_PinDirectionType Direction) {
  ...
  else if (Port_status == PORT_UNINITIALIZED) {
  ...
  { 
    ...
    */
    ...
    if(((Port_Configuration[Pin].port_num == 3) && (Port_Configuration[Pin].pin_num == 7)) || ((Port_Configuration[Pin].port_num == 5 ) && (Port_Configuration[Pin].pin_num == 0))) {
    ...
    else if ( (Port_Configuration[Pin].port_num == 2) && (Port_Configuration[Pin].pin_num <= 3 ))  {
      return ; 
      /* DO NOTHING FOR JTAG PINS */  
    ...
    if(Port_Configuration[Pin].direction == PORT_PIN_IN) {
      
      CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET) , Port_Configuration[Pin].pin_num);   
      
      
    } 
    else if (Port_Configuration[Pin].direction == PORT_PIN_OUT) {
      
      SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET) , Port_Configuration[Pin].pin_num);    
...
void Port_RefreshPortDirection( void )
{
  ...
  for(uint8 i = 0; i < CONFIGURED_PORT_PINS ; i++){
    ...
    volatile uint32 * PortGpio_Ptr = NULL_PTR; /* point to the required Port Registers base address */
    volatile uint32 delay = 0; 
    if(Port_Configuration[i].Dir_changable == STD_ON) {
      /*
      * Switch case to determine the Base address of the Port from the attribute port_num in the struct ConfigPtr
      */
      switch(Port_Configuration[i].port_num)
  ...
  ************************************************************************************/   
  ...
  void Port_SetPinMode( Port_PinType Pin, Port_PinModeType Mode ) {
    ...
    if (Port_Configuration[Pin].Mode_changable == STD_OFF) {
    ...
    if (( Mode < MIN_MODE ) || (Mode>MAX_MODE)){
      ...
      */
      ...
      if(Port_Configuration[Pin].mode == PORT_PIN_MODE_ADC)
      {
        ...
        *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) |= ((0x0000000F & Port_Configuration[Pin].mode) << (Port_Configuration[Pin].pin_num * 4));
        SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIGITAL_ENABLE_REG_OFFSET) , Port_Configuration[Pin].pin_num);        /* Set the corresponding bit in the GPIODEN register to enable digital functionality on this pin */
      }
  }
#endif
